...
class Authentication
{
    ...
    public function signup($email = '', $password = '', $confirm_password = ''){
        ...
        // check to make sure password & confirm password is the same
        if(
            !empty($password) && //$password is not empty
            !empty($confirm_password) && //$confirm_password is nnot empty
            $password != $confirm_password
        ){
            $error = "The password and confirm password fields should match";
        }

        // if not error, then proceed with sign up process
        if ( !empty( $error ) )
        return $error;
